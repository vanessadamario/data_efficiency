...
def plot_validation_curves(df, root_path,
                           result_path,
    ...
    n_training_list = list(set(df['n_training']))
...
def plot_auc_across_scenarios_and_repetitions(root_repetitions_lst,
    ...
    :param split_scenario_4: if False, the three scenarios are plotted together
    """
    scenario_lst = [1, 2, 4]
    ...
    for arch_ in arch_lst:
        ...
        plt.tight_layout()
        plt.savefig(join(results_path, 'MNIST_AUCs_%s.pdf' % arch_))
        plt.close()


def plot_auc_across_repetitions_split_scenarios(root_repetitions_lst, results_path):
    """ Plot for same scenario. We change data dimensions and amount of data.
    Here we report the test loss and the test accuracy.
    ...
    test_accuracy = np.zeros((len(arch_lst),
                              len(scenario_lst),
                              dataset_dim_array.size,
                              n_training_array.size,
                              len(root_repetitions_lst)))
    aucs_mean = np.zeros((len(arch_lst),
                          len(scenario_lst),
                          dataset_dim_array.size))
    aucs_std = np.zeros((len(arch_lst),
                         len(scenario_lst),
                         dataset_dim_array.size))

    for id_arch_, arch_ in enumerate(arch_lst):
        experiment_keys['architecture'] = arch_
            ...
            for id_s_, s_ in enumerate(scenario_lst):
                ...
                for id_d_, d_ in enumerate(dataset_dim_array):
                    ...
                    for id_n_, n_ in enumerate(n_training_array):
                        ...
                        best_id = search_best_id(root_path_, index_list)
                        ls, ac = np.load(join(root_path_, 'train_%i/test.npy' % best_id))
                        test_accuracy[id_arch_, id_s_, id_d_, id_n_, id_r_] = ac

        # print(test_accuracy)
        # max_auc = n_training_array[-1] * 2  # output classes
        ...
        for id_s_, s_ in enumerate(scenario_lst):
            ...
            for id_d_, _ in enumerate(dataset_dim_array):
                tmp_aucs = []
                for id_r_, _ in enumerate(root_repetitions_lst):
                    tmp_aucs.append(auc(n_training_array, test_accuracy[id_arch_, id_s_, id_d_, :, id_r_])/
                                    (n_training_array[-1] - n_training_array[0]))
                aucs_mean[id_arch_, id_s_, id_d_] = np.mean(np.array(tmp_aucs))
                aucs_std[id_arch_, id_s_, id_d_] = np.std(np.array(tmp_aucs))

    fig, ax = plt.subplots(figsize=(7, 4))
    plt.rc('xtick', labelsize=15)
    plt.rc('ytick', labelsize=15)
    for id_arch_, arch_ in enumerate(arch_lst):
        for idkk, kk in enumerate([1, 2]):  # scenario 1 and 2
            ax.errorbar(dataset_dim_array,
                        aucs_mean[id_arch_, idkk],
                        aucs_std[id_arch_, idkk],
                        marker='s',
                        linewidth=3,
                        label=dct_scenario_labels[kk] +', ' + arch_,
                        color='C%s' %idkk,
                        alpha=kk/2.
                        )
    ax.set_xlabel('image dimensions', fontsize='xx-large')
    ...
    ax.legend(fontsize='x-large')
    plt.tight_layout()
    plt.savefig(join(results_path, 'intemplate_scenario12_MNIST_AUCs_%s.pdf' % arch_))
    plt.close()
    ...
    ax.set_xlabel('dimension of the MNIST samples', fontsize='xx-large')
    ax.set_ylabel('AUC', fontsize='xx-large')
    ax.legend(fontsize='x-large')
    plt.tight_layout()
    plt.savefig(join(results_path, 'intemplate_scenario4_MNIST_AUCs_%s.pdf' % arch_))
    plt.close()
...
def main():
    ...
    if repetitions == 1:
        ...
        plot_auc_across_repetitions_split_scenarios(lst_root_path,
                                                    results_path=path_avg_plot)
        # for s_ in scenario_list:
            # for a_ in architecture_list:
