...
# TODO this is for fixed batch size
def plot_validation_curves(df, root_path, result_path, dataset='standardized_MNIST_dataset',
                           scenario=1, dataset_dimension=28,
                           architecture='FC', epochs=50):
    """ Plot for same scenario and data dimensions.
     ...
     :param dataset_dimension: int, dimension of the mnist - dataset
    ...
    n_training_list = list(set(df['n_training']))  # for all possible n_training
    output_path = join(result_path, dataset, architecture, 'validation')
    os.makedirs(output_path, exist_ok=True)  # we build the validation folder
    ...
    for n_ in sorted(n_training_list):
        ...
        # we want to plot the very best results
...
def plot_convergence_time(df, root_path, result_path, dataset='standardized_MNIST_dataset',
                           scenario=1, dataset_dimension=28,
                           architecture='FC', epochs=50):
    """ Plot for same scenario and data dimensions.
     Each curve represents the model trained with different amount of data.
    ...
    n_training_list = sorted(list(set(df['n_training'])))  # for all possible n_training
    output_path = join(result_path, dataset, architecture, 'convergence')
    os.makedirs(output_path, exist_ok=True)  # we build the validation folder
    ...
    ax[0].loglog(n_training_list, tr_lss_curve, 'o-', label='tr')
    ax[0].loglog(n_training_list, vl_lss_curve, '*-', label='vl')  # validation loss
    ax[0].set_xlabel('#n per class', fontsize='xx-large')
    ax[0].set_ylabel('cross entropy', fontsize='xx-large')
    ax[0].legend(fontsize='x-large')
    ax[1].semilogx(n_training_list, tr_acc_curve, 'o-', label='tr')
    ax[1].semilogx(n_training_list, vl_acc_curve, '*-', label='vl')  # validation accuracy
    ...
    fig, ax = plt.subplots()
    ax.semilogx(n_training_list, convergence_time, '*-')
    ...
    plt.tight_layout()
    plt.savefig(join(output_path, 'convergence_%i_dim_%i.pdf' % (scenario,
                                                                 dataset_dimension)))
    plt.close()


    fig, ax = plt.subplots()
    ax.semilogx(n_training_list, number_epochs, '*-')
    ax.set_xlabel('#n per class', fontsize='xx-large')
    ax.set_ylabel('#epochs', fontsize='xx-large')
    ax.set_title('convergence time', fontsize='xx-large')
    plt.tight_layout()
    plt.savefig(join(output_path, 'epochs_%i_dim_%i.pdf' % (scenario,
                                                            dataset_dimension)))
    plt.close()


def plot_convergence_time_per_batch(df, root_path, result_path,
                                      dataset='standardized_MNIST_dataset',
                                      scenario=1, architecture='FC',
                                      epochs=200, data_dim=28):
    ...
    :param data_dim: None or list, dataset dimensionality
    """
    experiment_keys = {'scenario': scenario,
                       'dataset_name': dataset,
                       'dataset_dimensions': data_dim,
                       'architecture': architecture,
                       'epochs': epochs}
    ...
    for n_tr in n_training_list:
        ...
        batch_size_per_n = sorted(list(set(df_['batch_size'])))
        ...
        for bs_ in batch_size_per_n:
            experiment_keys['batch_size'] = bs_
            index_list = generate_bm(df, experiment_keys=experiment_keys)['id'].values
        ...
        del experiment_keys['batch_size']
    ...
    fig.tight_layout(rect=[0, 0.03, 1, 0.95])
    plt.savefig(join(output_path, 'convergence_fixed_n_%i_dim_%i.pdf' % (scenario,
                                                                         data_dim)))
    plt.close()


def plot_performance_time_fixed_batch(df, root_path, result_path,
                                      dataset='standardized_MNIST_dataset',
                                      scenario=1, architecture='FC',
                                      epochs=200, data_dim=28):
    """ Plot for same scenario. We change data dimensions and amount of data.
    ...
    output_path = join(result_path, dataset, architecture, 'fixed_batch')
    os.makedirs(output_path, exist_ok=True)  # we build the validation folder

    for n_tr in n_training_list:
        experiment_keys['n_training'] = n_tr  # for each n
        index_list = generate_bm(df, experiment_keys=experiment_keys)['id'].values
        df_ = df.iloc[index_list]
        batch_size_per_n = sorted(list(set(df_['batch_size'])))
        ...
        for bs_ in batch_size_per_n:
            ...
            index_list = generate_bm(df, experiment_keys=experiment_keys)['id'].values
            df_ = df.iloc[index_list]
            learning_rate_list = sorted(list(set(df_['learning_rate'])))

            val_acc = []
            for lr_ in learning_rate_list:
                experiment_keys['learning_rate'] = lr_
                # there is just one now
                best_id = generate_bm(df, experiment_keys=experiment_keys)['id'].values
                df_hist = pd.read_csv(join(root_path, 'train_%i/history.csv' % best_id))  # we retrieve id
                val_acc.append(df_hist['val_accuracy'].values[-1])

            del experiment_keys['learning_rate']
            del experiment_keys['batch_size']
        ...
        plt.tight_layout()
        plt.savefig(join(output_path, 'scenario_%i_dim_%i_ntr_%i.pdf' % (scenario,
                                                                         data_dim,
                                                                         n_tr)))


def plot_test_curves(df, root_path, result_path, dataset='standardized_MNIST_dataset',
                     scenario=1, architecture='FC', epochs=50, data_dim=None):
    """ Plot for same scenario. We change data dimensions and amount of data.
    Here we report the test loss and the test accuracy.
    :param df: flattened dataframe,
    :param root_path: path where to retrieve results
    ...
    dataset_dim_list = data_dim if data_dim is not None else sorted(list(set(df['dataset_dimensions'])))
    ...
    for i_, dim_ in enumerate(dataset_dim_list):
        ...
        ax[1].semilogx(n_training_list, ac_test[i_], label='dim %i' % dim_)
...
def main():
    ...
    if epochs == 200:
        path_plots = join(path_plots, 'epochs_200')
    os.makedirs(path_plots, exist_ok=True)

    scenario_list = [1]
    data_dim_list = [28]
    architecture_list = ['FC']
